 ...
 * 3.2.5 UART
 * 3.3.1 done
 * 3.3.2 Ino longer than 15 ticks
*/
...
int main() {
	...
	xTaskCreate(CircleDisappear, "CircleDisappear", 1000, NULL, 6, &CircleDisappearHandle);					//3.2.2 --> set to different priority
	...
	/*
        ...
        CircleDisappearStaticHandle = xTaskCreateStatic(
                      CircleDisappearStatic,       // Function that implements the task.
                      "CircleDisappearStatic",          // Text name for the task.
...
/*------------------------------------------------------------------------------------------------------------------------------*/
/* ToDo: implement Task in scheduler
void DisplayFPS() {
	...
	while (1)
	{
		if(TickPerHz < xTaskGetTickCount() - StartTick)
		{
			TickPerHz++;
		}
		else
...
void PriorityTwoTask() {
	...
	while(1){
		...
		//if( !((xTaskGetTickCount() - exercise_3_3_2_start) % 2) ){
    		...
    		//this  should evaluate as true every 2 ticks for task 2 (not tested)
